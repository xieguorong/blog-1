...
@section('content')
    ...
    <div class="template">
        <div class="box">
            ...
            <ul>
                @foreach($pics as $pic)
                    <li><a href="{{ url('a/'.$pic->art_id) }}" target="_blank"><img src="{{ url($pic->art_thumb) }}"></a><span>{{ $pic->art_title }}</span></li>
                @endforeach
            </ul>
        </div>
    </div>
    <article>
        ...
        <div class="bloglist left">
            @foreach($articles as $article)
                ...
                <p class="dateview"><span>{{ date('Y-m-d', $article->art_time) }}</span><span>作者：{{ $article->art_editor }}</span>
                </p>
            @endforeach
            <div class="page">
                {{ $articles->links() }}
